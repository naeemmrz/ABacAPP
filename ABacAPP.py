...
page_title='ABacAPP', page_icon=PIL.Image.open('logo.png'), layout='wide', initial_sidebar_state='auto')
...
if selected == "Home":
	...
	st.markdown("<h6 style='text-align: justify; color: #C0C0C0;'> The β-lactamases model is based on the Random Forest regressor and the Klekota Roth Count features (shape=4860), the model was trained on 2045 datapoints and achives R², MSE, RMSE, and MAE of 0.87, 0.42, 0.65, and 0.44 respectively (LOOCV on the test set, CV=10).\n The DNA Gyrases model is based on the K-Nearest Neighbors regressor and the Circular Fingerprint features (radius=2 and shape=1024), the model was trained on 813 datapoints and achives R², MSE, RMSE, and MAE of 0.82, 0.67, 0.80, and 0.36 respectively (LOOCV on the test set, CV=10).\n The Bacterial Dihydrofolate Reductase model is based on the Random Forest regressor and the Klekota Roth Count features (shape=4860), the model was trained on 306 datapoints and achives R², MSE, RMSE, and MAE of 0.81, 0.74, 0.82, and 0.54 respectively (LOOCV on the test set, CV=10).\n The Penicillin-Binding Proteins model is based on the Extra Trees regressor and the Substructure Fingerprint Count features (shape=307), the model was trained on 132 datapoints and achives R², MSE, RMSE, and MAE of 0.88, 0.22, 0.44, and 0.24 respectively (LOOCV on the test set, CV=10).\n </h6>", unsafe_allow_html=True)
	st.write("\n")
	st.write("\n")
	st.write("\n")
